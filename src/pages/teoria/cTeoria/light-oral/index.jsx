...
export default function LightOralPage() {
      ...
      try {
        ...
        try {
          const [totalSaved, totalWrong, scoresObj] = await Promise.all([
            countAllBookmarks("cTeoria", "light"),
            countAllWrongAnswers("cTeoria", "light"),
            loadAllTypeScores("cTeoria", "light"),
          ]);
  ...
  if (!quizData.cTeoria || !quizData.cTeoria.light) {
    return (
  ...
  const quizNumbersObject = quizData.cTeoria.light;
      ...
      <Container sx={{ py: "30px", overflow: "hidden" }}>
        <Box display="flex" justifyContent="center" mb={2}>
          <Button
            variant="contained"
            color="primary"
            onClick={() => router.push(`/teoria/cTeoria/light-oral/random`)}
            sx={{
              fontWeight: "700",
              p: "10px 20px",
        ...
        >
          <StatisticsCard
            ...
            path={`/teoria/cTeoria/light-oral/saved`}
          ...
          <StatisticsCard
            ...
            path={`/teoria/cTeoria/light-oral/wrong`}
          ...
          {quizNumbers.map((quizNumber) => {
            ...
            return (
              <Grid item xs={12} sm={6} md={4} lg={3} key={quizNumber}>
                <QuizCard
                  quizName="أسئلة التؤوريا شحن خفيف شفوي"
